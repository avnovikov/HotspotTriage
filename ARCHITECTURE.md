...
|-----------------------|----------------------------------------------------|-------|
| `analyze`             | `_analyze_repository` + `_initialize_repository` + `revision_cache` | Block granularity only; **double** `build_block_stats` (see §9.1); publishes to dashboard; local repos record `revisions.pkl` snapshots (`head_sha`, optional `before_sha` / `after_sha`) |
