...
|-----------------------|----------------------------------------------------|-------|
| `analyze`             | `_analyze_repository` + `_initialize_repository` + `revision_cache` | Block granularity only; **double** `build_block_stats` (see §9.1); publishes to dashboard; JSON includes **`metadata`** (provenance, filters, config fingerprint); local repos record `revisions.pkl` snapshots (`head_sha`, optional `before_sha` / `after_sha`); when **`similarity`** is omitted, DeepCSIM defaults **off** if **`filter`** is set, **on** for whole-repo runs |
