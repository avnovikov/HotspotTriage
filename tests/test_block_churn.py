...
from code_complexity_py import block_churn
...
def test_cache_round_trip(tmp_path: Path, monkeypatch):
    ...
    cache_a = Cache(cache_path_for(repo))
    out_a = block_churn.compute_many(repo, requests, None, None, cache_a)
    cache_a.save()
    assert (path := cache_path_for(repo)).exists()
    ...
    cache_b = Cache(cache_path_for(repo))

    def fail(*a, **kw):
        raise AssertionError("cache miss — compute_one should not be called")

    monkeypatch.setattr(block_churn, "compute_one", fail)
...
def test_cache_invalidates_when_file_changes(tmp_path: Path, monkeypatch):
    ...
    cache = Cache(cache_path_for(repo))
    ...
    cache2 = Cache(cache_path_for(repo))
